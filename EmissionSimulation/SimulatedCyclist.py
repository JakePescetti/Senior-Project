...
def EngineOn():
    ...
    traci.start([sumoBinary, "-c", "bric.sumocfg",
                             "--full-output", "fulloutput-sim/tripinfo-"+str(waitCount)+"-"+timeNow+".xml", "--emission-output", "emissions-sim/emissions-"+str(waitCount)+"-"+timeNow+".xml"]) #may need to add  "--ignore-route-errors"
    run()
